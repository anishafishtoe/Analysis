...
#mixed effects models with interaction and random slopes
...
month=lmer(meanCrossings~month+(1+moonPhase|fStn), REML=FALSE, data=mAct.df)
...
microhab.moonphase.int=lmer(meanCrossings~FeedingTrayPosition+habitat+moonPhase+FeedingTrayPosition*moonPhase+(1|fStn), REML=FALSE, data=mAct.df)
habitat.microhab.int=lmer(meanCrossings~FeedingTrayPosition+habitat+moonPhase+FeedingTrayPosition*habitat+(1|fStn), REML=FALSE, data=mAct.df)
habitat.moonph.int=lmer(meanCrossings~FeedingTrayPosition+habitat+moonPhase+moonPhase*habitat+(1|fStn), REML=FALSE, data=mAct.df)
int.full=lmer(meanCrossings~FeedingTrayPosition+habitat+moonPhase++month+FeedingTrayPosition*habitat*moonPhase+(1|fStn), REML=FALSE, data=mAct.df)
int=lmer(meanCrossings~FeedingTrayPosition*habitat*moonPhase+(1|fStn), REML=FALSE, data=mAct.df)
anova(act.int.nullr, microhabitat,habitat, moonPhase, month, habitat.microhab, microhab.moonph,habitat.moonph, habitat.microhab.moonph, habitat.microhab.moonph.month, microhab.moonphase.int, habitat.moonph.int, habitat.microhab.int, int.full)
...
library(car)
full.model=lmer(meanCrossings~FeedingTrayPosition+habitat+moonPhase+month+FeedingTrayPosition*habitat+FeedingTrayPosition*moonPhase+moonPhase*habitat+FeedingTrayPosition*moonPhase*habitat+(1|fStn), data=mAct.df, REML=FALSE)
Anova(full.model)
